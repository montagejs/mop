
var Path = require("path");

var ESCAPE_REG = /[-[\]{}()*+?.\\^$|,#\s]/g;
function escapeRegExp(pattern) {
    return pattern.replace(ESCAPE_REG, "\\$&");
}

var globToRegExpCache = {};
var ESCAPE_GLOB_REG = /(\*\*\/)|(\*)|(\?)|(.)/g;
function globToRegExp(pattern) {

    // Cache pattern
    if (globToRegExpCache.hasOwnProperty(pattern)) {
        return globToRegExpCache[pattern];
    }

    var inner = pattern.replace(
        ESCAPE_GLOB_REG,
        function (_, ss, s, q, other) {
            if (ss) {
                return "([^/\\\\]*(?:/|\\\\))*";
            } else if (s) {
                return "[^/\\\\]*?";
            } else if (q) {
                return "[^/\\\\]";
            } else {
                return escapeRegExp(other);
            }
        }
    );

    var outer = new RegExp("^" + inner + "$");
    return (globToRegExpCache[pattern] = outer);
}

function testRegExp(expression) {
    return function (text) {
        return expression.test(text);
    };
}

exports.compileGlob = compileGlob;
function compileGlob(pattern) {
    return testRegExp(globToRegExp(pattern));
}

exports.compileGlobs = compileGlobs;
function compileGlobs(patterns) {
    var test = returnFalse;
    patterns.forEach(function (pattern) {
        test = composeOr(test, compileGlob(pattern));
    });
    return test;
}

exports.returnFalse = returnFalse;
function returnFalse() {
    return false;
}

exports.composeOr = composeOr;
function composeOr(a, b) {
    return function () {
        return a.apply(this, arguments) || b.apply(this, arguments);
    };
}

var isHidden = testRegExp(globToRegExp("**/.*"));
var defaultExclude = [
    "**/README",
    "**/CHANGES",
    "**/LICENSE",
    "**/*.txt",
    "**/*.md"
];

exports.makeFilter = makeFilter;
function makeFilter(location, excludeGlobs, config) {
    var exclude = isHidden;
    (excludeGlobs || []).concat(defaultExclude).forEach(function (pattern) {
        //console.log('pattern', pattern, globToRegExp(pattern));
        exclude = composeOr(exclude, testRegExp(globToRegExp(pattern)));
    });
    return function (name, stat) {
        //console.log("Filter file", Path.relative(location, name), 'Relative to', location);
        if (Path.basename(name) === "node_modules") {
            return null;
        } else if (exclude(Path.relative(location, name))) {
            if (stat.isDirectory()) {
                return null;
            } else {
                return false;
            }
        } else if (stat.isDirectory()) {
            return false;
        } else {
            return true;
        }
    };
}

