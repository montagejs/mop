...
function transform(appPackage, config) {
    ...
    try {
        var appLocation = appPackage.location.replace(/^file:\/\//, '');
        config.babelConfig = require(path.join(appLocation, 'babel.config.js'));
    } catch (err) {
        config.babelConfig = require('./babel.config.default.js');
    }
