...
function bundleHtml(file, config) {
    ...
    } else if (package.hasPackage({name: "mr"})) {
        ...
        Object.forEach(loaderPackage.modules, function (loaderModule, id) {
            ...
            if (loaderModule.location !== void 0 && loaderModule.type === "javascript") {
                ...
                if (config.files[relocation]) {
                    ...
                    var parts = config.files[relocation].buildLocation.split("packages"),
                        appCacheLocation;
                    // console.log(parts.length === 2, loaderModule.location);
                    if (parts.length === 2) {
                        // config.files[relocation].remove = true;
                        appCacheLocation = "**/packages" + parts[1];
                        bundledModules.push(appCacheLocation);
                    }
                }
            }
        });
