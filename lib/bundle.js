...
function bundleHtml(file, config) {
    ...
    try {
        ...
        console.warn("(warn) HTML parse error: " + file.path);
...
function verifyPackageLocation(element, file, config) {
    ...
    if (element.hasAttribute("data-package")) {
        packageLocation = URL.resolve(file.buildLocation, element.getAttribute("data-package"));
        if (packageLocation !== file.package.buildLocation) {
            console.warn("(warn) Montage bootstrapping script reports innacurate data-package location.");
            console.warn("In file: " + file.path);
            console.warn("Expected: " + file.package.buildLocation);
            console.warn("Actual: " + packageLocation);
        ...
        if (packageLocation !== file.package.buildLocation) {
            console.warn("(warn) Montage bootstrapping script needs a data-package attribute.");
            console.warn("In file: " + file.path);
            console.warn("Expected: " + URL.relative(file.location, file.package.location));
...
function collectBundle(loader, bundler, config, bundle) {
    ...
    Object.forEach(loader.packages, function (loaderPackage, location) {
        if (loaderPackage.bundled) return;
    ...
    Object.forEach(loader.packages, function (loaderPackage, location) {
        ...
        Object.forEach(loaderPackage.modules, function (loaderModule, id) {
            ...
            if (loaderModule.bundled) return;
...
function createPreloadBundles(phases, file, config) {
    return phases.map(function (phase, phaseNumber) {
        return phase.map(function (shard, shardNumber) {
            ...
            shardFile.utf8 += '\nbundleLoaded(' + JSON.stringify(shardFile.relativeLocation) + ')';
            return shardFile;
        });
    });
}
