...
function loadDeepPackages(package, memo, redirect) {
    memo = memo || {};
    redirect = redirect || {};
    ...
    return Promise.all(Object.keys(mappings).map(function (id) {
        ...
        if (!memo[location]) {
            ...
            .catch(function (err) {
                // Load may have failed because the package is installed flat
                // instead of nested.
                if (mapping.strategy === "nested" && mapping.flatLocation) {
                    memo[mapping.flatLocation] = memo[mapping.location];
                    redirect[mapping.location] = mapping.flatLocation;
                    mapping.strategy = "flat";
                    return package.loadPackage(mapping.flatLocation);
                } else {
                    throw err;
                }
            })
            .then(function (dependency) {
                return loadDeepPackages(dependency, memo, redirect);
            });
            memo[location] = loading;
            return loading;
        }
    })).then(function () {
        Object.keys(mappings).map(function (id) {
            var mapping = mappings[id],
                redirectForMapping = redirect[mapping.location];
            if (redirectForMapping) {
                mapping.location = redirectForMapping;
            }
        });
        return package;
    });
