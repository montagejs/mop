...
function loadMrScript(element, file, config) {
    ...
    if (!element.hasAttribute("data-module")) {
        console.warn("(error) Element needs data-module attribute in " + JSON.stringify(file.path));
        return;
    }
...
function bundleMrScript(bootstrapBundle, preloadFiles, element, file, config) {
    ...
    var plan = preloadFiles.map(function (phase) {
        return phase.map(function (shard) {
            console.log("Bundle:", shard.buildLocation, shard.utf8.length, "bytes");
    ...
    console.log("Bundle:", bundleFile.buildLocation, bundleFile.utf8.length, "bytes (mr bootstrap)");
