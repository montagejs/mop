...
function bundleMontageScript(bootstrapBundle, preloadFiles, element, file, config) {
    ...
    if (applicationPackageDescription.mainWorkerModule) {
        var bootstrapWorkerScriptLocation = applicationPackageDescription.bootstrapWorkerScript || "worker-bootstrap",
            ...
            workerImportRegexp = /importScripts\(\s*("|')node\_modules\/montage\/worker\.js\1\s*\)\s*[;,]?/;
        ...
        if (workerImportRegexp.test(bundleFile.utf8)) {
            bundleFile.utf8 = bundleFile.utf8.replace(workerImportRegexp,  "importScripts(\"" + toMontage + "worker.js" +"\"),");
        }
    }
    return null;
}
...
function prepareMontageWorkerContent(workerFile, workerParams) {
    ...
    if (!didPrepareMontageWorkerContent) {
        ...
        content = replaceOrPrependExpressionWith(content, montageImportRegexp, "importScripts(\"" + workerParams.bundleLocation + "\");");
